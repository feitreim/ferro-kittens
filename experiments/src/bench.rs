...
//!
//! **The clock itself is `examples/src/bench.rs`** and is included here rather
//! than copied. [`Shape`] and [`Timings`] cross the seam between the two crates
//! — `softmax::bench` and `layernorm::bench` are written against them in the
//! teaching crate and called from here — so one of the two files has to own
//! them, and it is the one whose kernels state their signatures. Everything
//! below is the sweep, which is this crate's job and not that one's.
...
/// intercept, and `experiments/README.md` §7 reports the fixed cost as a range
...
/// `experiments/README.md` §7 is a difference between two whole launches, and a
...
/// `cargo oxide run kittens-experiments -- bench`. Unlike the status table, this
